...
class navigation_demo(object):
    ...
    def _parse_args(self):
        ...
        project_base_path = str(repository_root() / 'inference')
        ...
        if not hasattr(self.args, 'result_dir'):
            self.args.result_dir = str(base_weights())

        if not hasattr(self.args, 'data_root'):
            self.args.data_root = str(repository_root() / 'data/raw')

        if not hasattr(self.args, 'clips_ckpt'):
            result_dir = getattr(self.args, 'result_dir', str(base_weights()))
        ...
        if not hasattr(self.args, 'explicit_dataset_folder'):
            self.args.explicit_dataset_folder = \
                str(repository_root() / 'data/raw/motionbricks-G1')
