...
本项目增加了中文交互界面、Windows 启动器，以及实验性的 UE 动画导出和自定义骨架训练入口。运行时推理、统一人形重定向和 CMC / Mover 接入仍待实现，具体能力以 [UE 插件说明](../Unreal/AILocomotionSystem/README.md) 为准。

## 建议学习顺序

1. 阅读 [动作表示](../motionbricks/docs/motion_representation.md)，理解骨架、Root Motion、局部姿态和特征布局。
2. 运行 [G1 交互演示](../motionbricks/scripts/interactive_demo_g1.py)，观察输入与生成动作的关系。
3. 阅读 [输入控制器](../motionbricks/motionbricks/motion_backbone/demo/controllers.py)。
4. 阅读 [逐帧推理](../motionbricks/motionbricks/motion_backbone/inference/motion_inference.py)。
5. 阅读 [VQ-VAE](../motionbricks/motionbricks/vqvae)，理解动作 Token。
6. 按 [训练说明](../motionbricks/README.md#训练) 使用合成数据理解训练任务；合成数据结果不能代表真实角色动作质量。
7. 按 [UE 插件说明](../Unreal/AILocomotionSystem/README.md) 接入实际动画资产，或参考 [通用数据集接入](../motionbricks/docs/adding_your_own_dataset.md)。
