...
```powershell
.venv/Scripts/python.exe data/tools/motion_pipeline.py scan-seed --source D:/BONES-SEED
.venv/Scripts/python.exe data/tools/motion_pipeline.py status
```
...
`data/tools/import_prepared_motions.py --source <训练目录> --library D:/MotionDataLibrary` 将完整快照复制到 `motions/prepared_<清单哈希>/`。
...
`data/tools/run_ue_pipeline_check.py --engine <UnrealEditor-Cmd.exe> --project <uproject> --work <请求目录>` 为每次 UE 检查建立唯一运行目录，要求退出码、日志和完整逐条报告共同通过。仍只检查轨道往返，不冒充 IK 重定向或发布门禁。
...
`data/tools/run_seed_queue.py --max-batches <数量>` 从数据库选择待处理动作，每批最多8条、累计约20000源帧。UE 导出器另有250000骨骼帧上限，故单动作超过3000帧先进入 `needs_segmentation`，不会截断；非已验证骨架头进入 `needs_skeleton_review`，不会丢弃。存在未收尾的 `retarget_running` 时拒绝重复处理，异常记录为 `retarget_failed` 后停止，需检查运行目录再恢复。
...
人工验收单次上限为 50 条（用户最新要求），与每 500 条处理停批的数量不同。异常项优先，占用后剩余名额用于固定随机抽样（至多其余动作的 5%，向上取整）。异常超过 50 条则拒绝生成超额名单，先修复/隔离，不能偷偷省略异常或把未审动作算通过。已保存且不超限的名单不重抽，旧结论保留。

当用户明确授权“自动技术验收后直接入库并清理”时，`run_overnight_seed_pipeline.py` 监督队列：每 500 条运行完整性初筛与训练格式回读，以 `accepted_imported` 正式发布最终无损轨道、共享骨骼和训练快照，再调用只允许精确哈希清单的清理器删除对应三种源表示、UE 批次资产和可重建工作文件。任何哈希、训练回读、状态、路径边界或磁盘门禁不通过都会使该批停止，不会删除文件或启动下一批；删除由持久清理日志逐文件记录。人工验收仍可用于事后抽检，但不再是自动推进的前置条件。
...
在仓库根目录运行 `.venv/Scripts/python.exe data/tools/motion_review_server.py --port 8765`，打开 `http://127.0.0.1:8765`。需要保持服务运行，不能直接双击 HTML 代替服务。

页面展示真实源 BVH 与 UE 导出动作的三维骨骼线框（不是蒙皮渲染），共用相机和时间轴。视口按 [Epic 的 Viewport Controls](https://dev.epicgames.com/documentation/unreal-engine/viewport-controls-in-unreal-engine) 实现主要导航：透视视图右键原地转向、右键配合 WASD/QE 飞行、右键配合滚轮调飞行速度，左键拖动前后移动并左右转向；Alt+左键绕焦点、Alt+右键推拉、中键或 Alt+中键平移、滚轮缩放；正交视图使用正交投影，右键平移、滚轮缩放。F 聚焦当前帧角色，“全段入镜”重置到整段动作范围。动画另有暂停、拖帧、逐帧和速度调整；这里的空格用于播放，不是 UE 的变换工具切换。坐标统一为厘米，不缩放身体；验收页固定显示未经对齐的导出 Root 坐标、保留高度的三维轨迹，以及逐帧 XYZ 和相对首帧位移。该坐标属于动画导出轨道，不等于关卡 Actor 的运行时世界 Location。
