...
def close(library,validation_path,cohort_path,commit=False):
    library=Path(library).resolve()
    validation_path=Path(validation_path).resolve()
    cohort_path=Path(cohort_path).resolve()
    if not validation_path.is_relative_to(library/'work') or cohort_path.parent!=library/'reviews':
        raise ValueError('回执路径越界')
    report=json.loads(validation_path.read_text(encoding='utf-8'))
    cohort=json.loads(cohort_path.read_text(encoding='utf-8'))
    screen_path=library/'reports/pending_quality_screen.json'
    if file_sha256(screen_path)!=cohort['screen_sha256']:
    ...
    target=library/'motions'/('seed_'+identity)
    ...
    try:
        ...
        if commit:
            ...
            write_json(target/'manifest.json',dict(schema_version=1,kind='uefn_motion_library',items=items,acceptance=plan['acceptance'],training_snapshot=str(snapshot)))
            for item in items:
            ...
            with catalog.db:
                for item in items:
                    catalog.db.execute("UPDATE motions SET state='accepted_imported',output_path=?,output_sha256=? WHERE id=? AND state='retargeted_pending_quality'",(str(target/item['file']),item['sha256'],item['id']))
                    catalog.db.execute('INSERT INTO events(motion_id,state,detail) VALUES(?,?,?)',(item['id'],'accepted_imported',json.dumps(dict(batch=identity,manifest=str(target/'manifest.json'),acceptance=plan['acceptance']))))
...
if __name__=='__main__':
    ...
    close(args.library,args.validation,args.cohort,args.commit)
