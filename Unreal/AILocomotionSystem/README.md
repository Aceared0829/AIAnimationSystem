...
> **当前状态：实验性实现，已完成一轮真实资产端到端验证。** UE 5.8.2 已完成插件打包与加载；本机 UEFN Mannequin 动画实际导出 1,728 段并预处理为 79 骨骼训练集，随后在 RTX 4070 Laptop GPU 上完成 VQ-VAE 与 Pose 各 1,000 步训练。模型导出、独立动作质量评估和 UE 运行时推理仍未实现，因此这些结果只证明训练数据通路和优化过程有效，不构成运行时动画质量承诺。
...
- 训练窗口至少 65 帧；默认 30 FPS 下建议至少 2.2 秒。默认拒绝较短动作；明确指定 `--short_clip_policy hold` 时，末帧保持到训练窗口作为姿态锚点，不循环或拼接原始动态轨迹。
...
2026-09-08：本机 UE 5.8.2 的 UHT、Editor 模块编译、DLL 链接及 `BuildPlugin` 打包通过。UEFN Mannequin 的 1,879 个 `AnimSequence` 中，1,728 段通过导出校验；预处理得到 79 骨骼、954 维、1,728 段真实训练集，其中 120 段短片段按末帧保持策略作为姿态锚点。RTX 4070 Laptop GPU 上的 1,000 步 VQ-VAE 和 1,000 步 Pose 训练均完成，检查点契约与数据集签名一致，参数有限。该结果没有独立留出集、动作重建视频或 UE 运行时评估。
