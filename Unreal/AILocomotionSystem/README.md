...
本仓库以 `codex/game-development-only` 作为主分支；本次插件与训练数据功能在 `codex/ue-animation-dataset` 开发分支维护，未经实测不视为主分支的稳定能力。
...
训练会保存 `config.yaml`、CSV 训练指标、周期检查点、`last.ckpt` 和正常完成时的 `final.ckpt`。使用 `--resume 路径` 恢复模型、优化器和训练步数；`--max_steps` 是恢复后总步数。Pose 续训仍需提供原 VQ-VAE。只加载可信的本地检查点。
...
- 拒绝叠加动画、非单位缩放、缺失语义骨骼、无效数值和每段超过配置上限的采样。当前最大 512 根骨骼、18000 帧，且每段帧数乘骨骼数不能超过 250000。
...
覆盖坐标与旋转基变换、非刚性平移拒绝、非法拓扑、四元数校验、混合骨架拒绝、统计量改变检查，以及 VQ-VAE/Root/Pose 的 CPU 小网络训练和 VQ-VAE 续训。测试数据为程序生成的刚性骨架动画，不等同于真实 Manny/Quinn 资产导出测试或动作质量验证。
