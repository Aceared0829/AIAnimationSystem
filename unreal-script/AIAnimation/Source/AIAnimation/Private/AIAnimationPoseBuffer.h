...
public:
	...
	void AddSource(int64 Index, TConstArrayView<FTransform> Pose, int32 AssetFrame = INDEX_NONE, double AssetTimeSeconds = -1.0, TConstArrayView<FTransform> FullSource = {});
	...
	/** 读取同一采样时刻的完整局部源姿态，用于同步模型未覆盖的辅助骨骼。 */
	bool ReadFullSource(double Position, TArray<FTransform>& OutPose) const;
